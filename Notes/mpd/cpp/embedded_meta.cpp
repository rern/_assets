...
ln -sf /bin/embedded_meta /bin/embedded.coverart
ln -sf /bin/embedded_meta /bin/embedded.lyrics
*/
#include <algorithm>
#include <cstdint>
#include <cstring>
...
AudioMetaReport parseWMA(std::ifstream& file) {
	...
	if (memcmp(asfHeaderGUID, fileGUID, 16) != 0) return r;
...
int main(int argc, char* argv[]) {
	std::string fullCmdPath = argv[0];
	size_t lastSlash = fullCmdPath.find_last_of("/\\");
	std::string baseCmdName = (lastSlash == std::string::npos) ? fullCmdPath : fullCmdPath.substr(lastSlash + 1);
	bool modeCoverArt = false;
	bool modeLyrics   = false;
	if (baseCmdName == "embedded_meta") {
		modeCoverArt = argc == 3;
		modeLyrics   = argc == 2;
	} else { // symlink
		modeCoverArt = baseCmdName == "embedded.coverart";
		modeLyrics   = baseCmdName == "embedded.lyrics";
	}
	if (argc < 2 || (modeCoverArt && argc < 3)) {
		std::cerr
			<< "Usage:\n" << baseCmdName << " FILE_SOURCE ";
		if (modeCoverArt) {
			std::cerr << "FILE_TARGET  # Extract coverart - auto append/corect .jpg/.png";
		} else {
			std::cerr << " # Extract lyrics to stdout";
		}
